...
initialize() {
	read -r -p "${cyan}Install submariner-prompt? ${reset}[y/N] " answer
...
setup_bash_prompt() {
	...
	echo "${green}Installing submariner-prompt...${reset}"
	...
	echo "Sourcing .submariner_prompt in .bashrc..."
	...
	printf '%s\nsource $HOME/.submariner_prompt' >> $HOME/.bashrc

	echo "Symlinking .submariner_prompt..."
	ln -sf "$(pwd)/.submariner_prompt" $HOME
	...
	echo "${green}submariner-prompt installed.${reset}"
	echo "Enter ${cyan}source ~/.bashrc${reset} to reload prompt"
	echo "Remember to install a ${cyan}Powerline Font${reset} and the ${cyan}FontAwesome${reset} font stack."
